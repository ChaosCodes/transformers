...
class GenerationMixin:
    ...
    def generate(
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        speculative_decoding_type: Optional[str] = 'hf',
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            if speculative_decoding_type == 'hf':
            ...
            elif speculative_decoding_type == 'deepmind':
                ...
                raise ValueError(f"Unknown speculative decoding type: {speculative_decoding_type}")
    ...
    def speculative_decoding(
            ...
            **sample** (depending on `do_sample`), assisted by a smaller model. Can be used for text-decoder, text-to-text,
            ...
            while True:
                ...
                assistant_model_logits = torch.cat(assistant_model_logits, dim=1)
                ...
                # 2.2. Process the new logits
                new_logits = outputs.logits[:, -candidate_length - 1 :]  # excludes the input prompt if present
                assistant_model_logits = assistant_model_logits[:, -candidate_length - 1 :]

                if len(logits_processor) > 0:
                    for i in range(candidate_length):
                        new_logits[:, i, :] = logits_processor(candidate_input_ids[:, : cur_len + i], new_logits[:, i, :])
                        assistant_model_logits[:, i, :] = logits_processor(candidate_input_ids[:, : cur_len + i], assistant_model_logits[:, i, :])
                if len(logits_warper) > 0:
                    for i in range(candidate_length):
                        new_logits[:, i, :] = logits_warper(candidate_input_ids[:, : cur_len + i], new_logits[:, i, :])
                        assistant_model_logits[:, i, :] = logits_warper(candidate_input_ids[:, : cur_len + i], assistant_model_logits[:, i, :])

                # 3. Obtain the next tokens from the original model logits.
                probs = new_logits[:, -candidate_length - 1 :, :].softmax(dim=-1)
                assistant_model_probs = assistant_model_logits[:, -candidate_length - 1 :, :].softmax(dim=-1)
                
                n_matches = 0
                r = torch.rand(candidate_length).cuda()
                for i in range(candidate_length):
                    # Sample 𝑟 from a uniform distribution.
                    contrastive_prob = probs[0][i][candidate_input_ids[0][i - candidate_length]] / \
                        assistant_model_probs[0][i][candidate_input_ids[0][i - candidate_length]]

                    if r[i] < min(1, contrastive_prob):
                        # accept the candidate token
                        n_matches += 1
                    else:
                        contrastive_sample_prob = torch.relu(probs[:, i, :] - assistant_model_probs[:, i, :])
                        # sample token based on contrastive_sample_prob
                        token_sample_from_contrastive_prob = torch.multinomial(contrastive_sample_prob, num_samples=1).squeeze(1)[None, :]
                        break
                ...
                accepted_tokens_num += n_matches

                # 5.2. Get the valid continuation, after the matching tokens
                valid_tokens = torch.cat([candidate_input_ids[:, : n_matches], token_sample_from_contrastive_prob], dim=-1)
                input_ids = torch.cat((input_ids, valid_tokens), dim=-1)
                if streamer is not None:
                    streamer.put(valid_tokens.cpu())
                new_cur_len = input_ids.shape[-1]
            ...
            print('accepted_tokens_num', accepted_tokens_num)
            print('accepted rate', float(accepted_tokens_num) / (input_ids.shape[-1] - prompt_len), '%')
